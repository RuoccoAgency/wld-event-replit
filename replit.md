...
## Recent Changes
- 2026-04-30: Auto-seeding: seed-data.ts (21 cars + 76 images) + seed.ts + seedProductionDatabase() called on server startup; production DB populates automatically when tables are empty
- 2026-04-30: Supabase form integration: @supabase/supabase-js installed; server/supabase.ts client (service role); POST /api/bookings saves to booking_requests table; POST /api/partner-applications saves to partner_applications table; BookingForm.tsx onSubmit now calls /api/bookings; become-partner.tsx fully controlled with state + file upload via Object Storage presigned URLs
